...
public interface MeuPedidoRepository extends JpaRepository<MeuPedido, String> {
	...
	@Query(value = "select new br.unicesumar.adsis4s2021.meu.lucas.duranteAsAulas.pedido.dto.MeuPedidoDTO("
			+ "mp.id, mp.numero, mp.emitidoEm, mc.id, mc.nome) "
			+ "from MeuPedido mp "
			+ "inner join mp.cliente mc")
	List<MeuPedidoDTO> encontrarPedidoComoDTO();
	
	@Query(value = "select new br.unicesumar.adsis4s2021.meu.lucas.duranteAsAulas.pedido.dto.MeuItemPedidoDTO("
